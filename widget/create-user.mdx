...
ZBD checks the supplied email during widget-user creation. Creation is rejected
when the email or its domain is blocked. Email aliases do not bypass the check:
`player+test@example.com` is evaluated as `player@example.com` for an
email-level block.

Emails using the following country-code top-level domains are not supported:

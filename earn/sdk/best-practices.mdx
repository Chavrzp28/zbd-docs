...
`Init` reports its result on the `completion` object. Handle each outcome distinctly — never fail silently. In short: **maintenance** and **unsupported region** have no retry path (hide the rewards UI, message only players with a balance); everything else (network, VPN/proxy, attestation) gets `ShowModal()` so the player can see why and retry.

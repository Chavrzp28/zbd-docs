...
If you have not been given an App ID, reach out to your account manager.
...
<Note>
This will allow the user to start earning, however **in order to withdraw they will need a ZBD account linked to your game**.

The easiest way to do this is to present the ZBD modal. If the player doesn't have an account linked yet, the modal guides them through creating or linking one the first time it opens.

**Timing matters:** present the modal when the player taps their rewards balance or a claim prompt — once there's a balance worth claiming — never at cold launch or before they've earned anything. Registration then reads as delivering a reward the player already owns, not as sign-up. See the [Player Communication Guide](/earn/sdk/player-comms).
</Note>
...
`ShowModal()` presents the ZBD modal. If the player isn't signed in yet, it starts by guiding them through creating or linking their ZBD account; once signed in, they go straight to their balance.

Users can **view their balance** and **initiate a withdrawal** from this modal.

...
description: 'Bring players back to keep earning or cash out, with SDK-scheduled local reminders — no push infrastructure required.'
icon: 'bell'
---

A player who leaves with an uncashed balance is the easiest player to win back. Local reminders let you reach them — *"you have earnings waiting"*, *"you're 80% of the way to an Amazon card"* — without any push infrastructure.
...
<Note>
Reminders ship in the next SDK release after v1.1.7. On earlier versions the SDK has no reminder API.
</Note>
...
ZBDController.Instance.ConfigureReminders(new ZBDReminderConfig
{
    ...
    rules = new[]
    {
        ...
        {
            when = s => s.balance > 0,
            title = "You have earnings to cash out",
            body = "You've got {balance} {currency} waiting. Come and cash out.",
        },
    }
});
```
...
The snapshot is **as of the player's last session, not a live read** — reminders are scheduled while the app is suspending, which is not a safe place for network calls, so the SDK caches this during play (after `Init` and after every withdrawal). You never refresh it yourself.

### Conditions beyond ZBD

`when` is an ordinary predicate, so a rule can be about anything in your game — the ZBD snapshot is offered, not imposed:

```csharp
new ZBDReminderRule
{
    when    = _ => myGame.UnopenedLootboxes > 0,
    title   = "Your lootboxes are waiting",
    bodyFor = _ => $"You have {myGame.UnopenedLootboxes} unopened lootboxes.",
},
```

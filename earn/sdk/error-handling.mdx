...
|-------|-------------|------------|
| VPN / ad blocker / private DNS blocking device verification | `completion.success = false` | Call `ShowModal()`. The modal shows an actionable message (disable the blocker or VPN and retry). Note: VPN *policy* detection does not fail init — it places the player in limited earnings mode instead. See [Fraud Prevention](/earn/sdk/fraud-prevention). |
...
|----------|-------------|-----------------|
| Init — VPN/ad-blocker interference | `success = false` | `ShowModal()`. Game continues. |

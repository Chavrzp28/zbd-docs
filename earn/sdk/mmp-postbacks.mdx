...
## Prerequisite: set your own player ID

Every approach below joins an MMP event to a ZBD player, and the join key is **your own internal player identifier**. Pass it when you initialize the SDK, or set it later with `SetUserId`:

```csharp
ZBDController.Instance.Init(userId, completion => { /* ... */ });
```

Without it there's nothing to match your MMP's user against, and none of the options below will work. See [Integration](/earn/sdk/integration#initialize-the-sdk).
...
Once limits track revenue, you can check a player's state before issuing a reward and scale it to what they can actually withdraw:
...
That tells you two things: `rewardsBalance` is what the player currently has, and `withdrawableLimit` is what they're allowed to cash out. Players with more headroom can be given larger rewards, because you know the revenue is there to back them.

...
The ZBD Widget is an embeddable iframe that runs the cashout experience for your users — OTP verification, identity verification (KYC), bank account linking, and bank payouts. Your backend creates widget users, funds their point balances, creates sessions, and handles its own end-user terms and disclosures before the widget opens.
...
The widget uses the country resolved from the user's IP before KYC. After KYC,
it uses the user's verified residential country to select the available bank
rail.

| Bank rail | Supported countries |
| --- | --- |
| ACH / USD bank | United States (`US`) |
| SCT / EUR bank | Austria (`AT`), Belgium (`BE`), Germany (`DE`), Denmark (`DK`), Estonia (`EE`), Spain (`ES`), Finland (`FI`), France (`FR`), Italy (`IT`), Lithuania (`LT`), Latvia (`LV`), Netherlands (`NL`), Poland (`PL`), Portugal (`PT`), and Sweden (`SE`) |

SCT is unavailable in Bulgaria (`BG`), Croatia (`HR`), Cyprus (`CY`), Czechia
(`CZ`), Greece (`GR`), Hungary (`HU`), Ireland (`IE`), Luxembourg (`LU`), Malta
(`MT`), Romania (`RO`), Slovakia (`SK`), Slovenia (`SI`), Switzerland (`CH`),
and every other country outside the SCT list above. Unsupported countries do not
receive the EUR/SCT bank rail.

The widget is not supported for users in Cuba (`CU`), Iran (`IR`), North Korea
(`KP`), or Syria (`SY`).
